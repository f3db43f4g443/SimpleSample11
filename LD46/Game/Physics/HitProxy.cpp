...
bool SweepTestPolygons( SHitProxyPolygon* a, SHitProxyPolygon* b, const CMatrix2D& transA, const CMatrix2D& transB, const CVector2& sweepOfs, SRaycastResult* pResult = NULL )
{
	...
	for( int i1 = 0, i2 = 0;; )
	{
		...
		if( fMaxDist > 0 )
		{
			...
			if( bIntersect )
			{
				float k = ( p0.x * p1.y - p0.y * p1.x ) / ( dir.x * p1.y + p0.x * dir.y - dir.y * p1.x - dir.x * p0.y );
				auto& intersect = intersects[nIntersects++];
				intersect.k = k;
				intersect.norm = norm * -1;
				intersect.hitPoint = l1 == n1 ? vertices1[l1] - dir * k : vertices2[l2] * -1;
	...
	if( nIntersects <= 1 )
		return false;
	SIntersect *pIn1, *pIn2;
	if( intersects[0].k < intersects[1].k )
	{
		pIn1 = &intersects[0];
		pIn2 = &intersects[1];
	}
	...
	if( pIn1->k >= fMaxDist || pIn2->k <= 0 )
		return false;
	if( pResult )
	{
		if( pIn1->k >= 0 )
		{
			pResult->normal = pIn1->norm;
			pResult->hitPoint = pIn1->hitPoint;
			pResult->fDist = pIn1->k;
		}
		else
		{
			SHitTestResult res1;
			HitTestPolygons( a, b, transA, transB, &res1 );
			pResult->normal = res1.normal * -1;
			pResult->normal.Normalize();
			pResult->hitPoint = res1.hitPoint2;
			pResult->fDist = 0;
		}
	}
	return true;
...
bool SHitProxyPolygon::Raycast( const CVector2& begin, const CVector2& end, const CMatrix2D& trans, SRaycastResult* pResult )
{
	...
	for( int i = 0; i < nVertices; i++ )
	{
		...
		if( fMaxDist > 0 )
		{
			...
			float k = ( vert1.x * vert2.y - vert1.y * vert2.x ) / ( dir.x * vert2.y + vert1.x * dir.y - dir.y * vert2.x - dir.x * vert1.y );
			auto& intersect = intersects[nIntersects++];
			intersect.k = k;
			intersect.norm = trans.MulVector2Dir( normals[i] );
			intersect.hitPoint = trans.MulVector2Pos( p0 + dir * k );
			if( nIntersects == 2 )
				break;
		}
	}
	...
	if( pResult )
	{
		if( pIn1->k >= 0 )
		{
			pResult->normal = pIn1->norm;
			pResult->hitPoint = pIn1->hitPoint;
			pResult->fDist = sqrt( fMaxDist ) * pIn1->k;
		}
		else
		{
			pResult->normal = pIn1->norm;
			pResult->hitPoint = trans.MulVector2Pos( p0 );
			pResult->fDist = 0;
		}
	}
	return true;
...
{
	static CBufFile g_buf[2];
	PackData( g_buf[0], true );
	obj0.PackData( g_buf[1], true );
	if( g_buf[0] == g_buf[1] )
	{
		g_buf[0].Clear();
		g_buf[1].Clear();
	...
	buf.Write( g_buf[0].GetBuffer(), g_buf[0].GetBufLen() );
	g_buf[0].Clear();
	g_buf[1].Clear();
	return true;
}
...
bool CHitProxy::HitTest( const CVector2& pos, float fRad )
{

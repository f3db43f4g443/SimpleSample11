...
public:
	CHitProxy() : m_pMgr( NULL ), m_pHitProxies( NULL ), m_pManifolds( NULL ), m_bLastPosValid( false ), m_velocity( 0, 0 )
	...
	void PackData( CBufFile& buf, bool bWithMetaData ) const;
	void UnpackData( IBufReader& buf, bool bWithMetaData, void* pContext );
	bool DiffData( const CHitProxy& obj0, class CBufFile& buf ) const;
	void PatchData( class IBufReader& buf, void* pContext );
	
	void CalcBounds();
	bool HitTest( const CVector2& pos, float fRad = 0 );
...
private:
	void Update( CHitProxy* pHitProxy, vector<CHitProxy*>* pVecOverlaps );

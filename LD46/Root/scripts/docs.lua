...
......]====]
	...
	{
		key = "_D3_ES",
		name = "Environment Sampling Data",
		content = function()
			local str = "Sampled data : "
			local n = 0
			for i = 1, 20, 1 do
				if EvaluateKeyInt( "%doc_unlocked_D3_ES_" .. tostring( i - 1 ) ) > 0 then
					n = n + 1
				end
			end
			str = str .. tostring( n ) .. [====[ / 20
============================================================

]====]
			for i = 1, 20, 1 do
				if EvaluateKeyInt( "%doc_unlocked_D3_ES_" .. tostring( i - 1 ) ) > 0 then
					str = str .. Sampling_Data[i]
				end
			end
...
function UnlockDoc( strName, bForceShow )
	if FEVT( GetKeyUnlockDoc( strName ) ) or bForceShow then

...
float4 vSizeAlign;
void Default2D_Vertex1( in float2 tex,
	in uint instID,
	out float2 outTex,
	out float2 outTex1,
	out float4 outPos )
{
	...
	float2 dPos = tex - vSizeAlign.zw;
	dPos = ( dPos.x * matX + dPos.y * matY ) * 2;
	outTex1 = vSizeAlign.zw + dPos / vSizeAlign.xy;
}

void VSDefault1( in float2 tex : Position,
	in uint instID : SV_InstanceID,
	out float4 outPos : SV_Position,
	out float2 outTex : TexCoord0,
	out float2 outTex1 : TexCoord1 )
{
	Default2D_Vertex1( tex, instID * 2, outTex, outTex1, outPos );
}
...
void VSDefaultExtraInstData1( in float2 tex : Position,
	in uint instID : SV_InstanceID,
	out float4 outPos : SV_Position,
	out float2 outTex : TexCoord0,
	out float2 outTex1 : TexCoord1,
	out float4 outInstData[EXTRA_INST_DATA] : ExtraInstData0 )
{
	Default2D_Vertex1( tex, instID * INST_DATA_SIZE, outTex, outTex1, outPos );

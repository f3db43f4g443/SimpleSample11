...
void CLevelGenNode1_3::GenBase()
{
	...
	while( nLen )
	{
		...
		do
		{
			nType = SRand::Inst().Rand( 0, nType );
		} while( nType && minSize[nType] > nLen );
	...
	for( auto p : vec )
	{
		...
		case 3:
		{
			...
			if( SRand::Inst().Rand() & 1 )
			{
				...
				GenSubArea( TRectangle<int32>( 0, nCurBegin, left, nCurHeight ) );
				GenSubArea( TRectangle<int32>( left, nCurBegin + h0 + h1, right - w2 - left, h2 ) );
				GenSubArea( TRectangle<int32>( right, nCurBegin, nWidth - right, nCurHeight ) );
				GenSubArea( TRectangle<int32>( left + w1, nCurBegin + h0, right - left - w1, h1 ) );
			}
			else
			{
				...
				GenSubArea( TRectangle<int32>( 0, nCurBegin, left, nCurHeight ) );
				GenSubArea( TRectangle<int32>( left, nCurBegin + h0, right - w2 - left, h1 ) );
				GenSubArea( TRectangle<int32>( right, nCurBegin, nWidth - right, nCurHeight ) );
			...
			m_wallChunks1.push_back( TRectangle<int32>( left, nCurHeight + h0 + h1 + h2, right - left, h3 ) );
		...
		case 4:
		{
			...
			m_wallChunks1.push_back( TRectangle<int32>( left1 + w1, nCurBegin, nWidth - left1 - w1, h0 ) );
	...
	{
		int8 nType = Min( rect.width, rect.height ) >= 4 ? eType_WallChunk0 : eType_WallChunk1;
...
void CLevelGenNode1_3::GenSubArea( const TRectangle<int32>& rect )
{
	...
	for( int i = 0; i < rects.size(); i++ )
	{
		...
		int32 nSplitWidth = Min( Max( curRect.height, curRect.width ) - nMinAreaSize * 2, SRand::Inst().Rand( 2, 5 ) );
		...
		if( !bVertical )
		{
			...
			if( !bFromLeft )
			{
				rect1.x = curRect.x + curRect.GetRight() - rect1.x;
				rect2.x = curRect.x + curRect.GetRight() - rect2.x;
				rectSplit.x = curRect.x + curRect.GetRight() - rectSplit.x;
			}
		}
		else
		{
			...
			if( !bFromLeft )
			{
				rect1.y = curRect.y + curRect.GetBottom() - rect1.y;
				rect2.y = curRect.y + curRect.GetBottom() - rect2.y;
				rectSplit.y = curRect.y + curRect.GetBottom() - rectSplit.y;
			}
		}
...
void CLevelGenNode1_3::FillLine( const TRectangle<int32>& rect )
{
	int32 nWidth = m_region.width;
	int32 nHeight = m_region.height;
	for( int j = rect.y; j < rect.GetBottom(); j++ )
	{
		...
		for( int i = rect.GetRight(); i < nWidth; i++ )
			m_gendata[i + j * nWidth] = eType_Wall;
	}
}
...
void CLevelGenNode1_3::GenObjs()
{
	...
	for( int i = 0; i < nWidth; i++ )
	{
		...
		{
			int32 nType = m_gendata[i + j * nWidth];
			int32 nType1 = m_gendata[i + ( j - 1 ) * nWidth];
			if( nType == eType_None && nType1 == eType_WallChunk1 )
				vec.push_back( TVector2<int32>( i, j ) );
		}
	}
	...
	LvGenLib::GenObjs( m_gendata, nWidth, nHeight, 0, eType_None, eType_Temp1 );

	int8 nTypes[4] = { eType_BlockRed, eType_BlockBlue };
	LvGenLib::FillBlocks( m_gendata, nWidth, nHeight, 32, 48, eType_Temp, nTypes, ELEM_COUNT( nTypes ) );
	int8 nTypes1[4] = { eType_Crate1, eType_Crate2 };

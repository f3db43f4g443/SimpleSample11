...
void CPostProcessDizzyEffect::Process( CPostProcessPass* pPass, IRenderTarget* pFinalTarget )
{
	...
	CVector4 t[2] =
	{
		CVector4( dt * 8.4, dt * 7.33, dt * 8.34, dt * 6.18 ) * 0.0f,
		CVector4( dt * 6.33, dt * 5.11, dt * 9.58, dt * 6.77 ) * 0.0f,
	};
	CVector4 tx[2] =
	{
		CVector4( 0.0123, 0.0023, 0.0103, -0.0031 ) * 0.0f * size.x,
		CVector4( -0.0078, -0.0042, -0.0103, 0.0051 ) * 0.0f * size.x,
	};
	CVector4 ty[2] =
	{
		CVector4( 0.0087, -0.0101, 0.0074, -0.0098 ) * 0.0f * size.y,
		CVector4( -0.0063, -0.0031, 0.0028, 0.0107 ) * 0.0f * size.y,
	};
	CVector4 tb[2] =
	{

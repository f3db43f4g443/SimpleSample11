...
public:
	...
	bool IsKey( uint32 nKey ) { return m_key.GetBit( nKey ) && !m_keyForceRelease.GetBit( nKey ); }
	bool IsKeyUp( uint32 nKey ) { return m_keyUp.GetBit( nKey ); }
	bool IsKeyDown( uint32 nKey ) { return m_keyDown.GetBit( nKey ); }
	void ForceKeyRelease( uint32 nKey ) { m_keyForceRelease.SetBit( nKey, true ); }

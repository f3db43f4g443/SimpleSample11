	...
	out float4 outPos )
{
	...
	float4 instData0 = g_insts[instID + 2];
	...
	vtex = vtex / 2048.0;
	float2 scr = ( outPos.xy * float2( 0.5, -0.5 ) ) * g_screenResolution;
	scr = ( scr + instData0.zw ) / instData0.xy;
	outTex = scr;
	outTex1 = float4( vtex, dTex );
}
...
#endif
#define INST_DATA_SIZE ( 3 + EXTRA_INST_DATA )
	...
	out float4 outInstData[EXTRA_INST_DATA] : ExtraInstData0 )
{
	...
	for( int i = 0; i < EXTRA_INST_DATA; i++ )
		outInstData[i] = g_insts[instID * INST_DATA_SIZE + i + 3];
}
...
{
	tex = frac( tex );
	outColor = Texture0.Sample( LinearSampler, tex1.xy + tex1.zw * tex );
}